---
title: "PyTorch源码解析系列 - 第04章：张量操作与算子系统"
collection: pytorch
...
graph TD
    Start(("开始")):::start
    Step1["定义算子<br/>native_functions.yaml"]:::step
    Step2["实现CPU kernel"]:::step
    Step3["实现CUDA kernel<br/>可选"]:::step
    Step4["添加autograd支持<br/>如果可微"]:::step
    Step5["编写测试"]:::step
    Step6["重新编译PyTorch"]:::step
    Step7["运行测试验证"]:::step
    End(("完成")):::end
    ...
    classDef start fill:#4A90E2,stroke:#2E5C8A,color:#fff,stroke-width:3px,font-size:14px
    classDef step fill:#3498DB,stroke:#2980B9,color:#fff,stroke-width:2px,font-size:12px
    classDef end fill:#27AE60,stroke:#1E8449,color:#fff,stroke-width:3px,font-size:14px
```
